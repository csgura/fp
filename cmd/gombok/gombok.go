...
func genPublicGetters(
	ctx TaggedStructContext,
	publicFields fp.Seq[metafp.StructField],
...
func processGetter(ctx TaggedStructContext, genMethod fp.Set[string]) fp.Set[string] {
	...
	if anno, ok := ts.Tags.Get("@fp.GetterPubField").Unapply(); ok {
		...
		genMethod = genPublicGetters(ctx, publicFields, anno, genMethod)
...
func genStringMethod(ctx TaggedStructContext, allFields fp.Seq[metafp.StructField], genMethod fp.Set[string]) fp.Set[string] {
	...
	if ts.Info.Method.Get("String").IsEmpty() && !genMethod.Contains("String") {
		...
		fm := iterator.FromSeq(printable).Map(func(f metafp.StructField) string {
		...
		fields := iterator.FromSeq(printable).Map(func(f metafp.StructField) string {
...
func genUnapply(ctx TaggedStructContext, allFields fp.Seq[metafp.StructField], genMethod fp.Set[string]) fp.Set[string] {
	...
	if allFields.Size() < max.Product {
		...
		if ts.Info.Method.Get("AsTuple").IsEmpty() && !genMethod.Contains("AsTuple") {
			...
			tp := seq.Iterator(allFields).Take(arity).Map(func(v metafp.StructField) string {
				return v.TypeName(w, workingPackage)
			}).MakeString(",")

			fields := iterator.FromSeq(allFields).Map(func(f metafp.StructField) string {
				return fmt.Sprintf("r.%s", f.Name)
	...
	if ts.Info.Method.Get("Unapply").IsEmpty() && !genMethod.Contains("Unapply") {
		...
		tp := iterator.FromSeq(allFields).Map(func(v metafp.StructField) string {
			return v.TypeName(w, workingPackage)
		}).MakeString(",")
...
func genBuilder(ctx TaggedStructContext, genMethod fp.Set[string]) fp.Set[string] {
	...
	privateFields.Foreach(func(f metafp.StructField) {
		...
		if ts.GetField(uname).IsEmpty() && !isMethodDefined(workingPackage, builderTypeName, uname) {
			ftp := f.TypeName(w, workingPackage)
	...
	if allFields.Size() < max.Product {
		...
		if !isMethodDefined(workingPackage, builderTypeName, "FromTuple") {
			...
			fields := iterator.Zip(iterator.Range(0, allFields.Size()), seq.Iterator(allFields)).Map(func(f fp.Tuple2[int, metafp.StructField]) string {
	...
	if !isMethodDefined(workingPackage, builderTypeName, "Apply") {
		...
		tp := seq.Iterator(allFields).Map(func(v metafp.StructField) string {
	...
	if !isMethodDefined(workingPackage, builderTypeName, "FromMap") {
		...
		fields := seq.Iterator(allFields).Map(func(f metafp.StructField) string {
			if f.FieldType.IsOption() {
				optionpk := w.GetImportedName(genfp.NewImportPackage("github.com/csgura/fp/option", "option"))
...
func genPublicWiths(
	...
	publicFields.Foreach(func(f metafp.StructField) {
		...
		if f.Embedded && f.FieldIndex == 0 && !f.FieldType.IsOption() {
...
func genWith(
	...
	func() {
		withName := fmt.Sprintf("With%s", publicName(fieldName))
		if ts.Info.Method.Get(withName).IsEmpty() && !genMethod.Contains(withName) {
	...
	fmine := ts.GetField(fieldName).OrElse(f)

	if f.FieldType.IsOption() && fmine.FieldType.ID == f.FieldType.ID {
		optiont := w.TypeName(workingPackage, f.FieldType.TypeArgs.Head().Get().Type)
		optionpk := w.GetImportedName(genfp.NewImportPackage("github.com/csgura/fp/option", "option"))

		fnName := "WithSome" + publicName(fieldName)
		if ts.Info.Method.Get(fnName).IsEmpty() && !genMethod.Contains(fnName) {
			...
			fmt.Fprintf(w, `
						...
						`, valuereceiver, fnName, optiont, valuereceiver, fieldName, optionpk)
			genMethod = genMethod.Incl(fnName)

		}

		fnName = "WithNone" + publicName(fieldName)
		if ts.Info.Method.Get(fnName).IsEmpty() && !genMethod.Contains(fnName) {
			...
			fmt.Fprintf(w, `
						...
						`, valuereceiver, fnName, valuereceiver, fieldName, optionpk, optiont)
			genMethod = genMethod.Incl(fnName)
...
func processWith(ctx TaggedStructContext, genMethod fp.Set[string]) fp.Set[string] {
	...
	if anno, ok := ts.Tags.Get("@fp.With").Unapply(); ok {
		allFields := applyFields(ts)
		privateFields := ts.Fields.FilterNot(metafp.StructField.Public)
		embedField := allFields.Filter(func(v metafp.StructField) bool {
			return v.Embedded
		})
		genMethod = genPrivateWiths(ctx, privateFields, genMethod)
		genMethod = genPublicWiths(ctx, embedField, anno, genMethod)
	}
...
type TaggedStructContext struct {
	...
	others         fp.Seq[metafp.TaggedStruct]
...
func (r TaggedStructContext) GeneratedWithFuncList(stType metafp.TypeInfo) fp.Seq[metafp.StructField] {
	for _, ts := range r.others {
		if ts.Info.ID == stType.ID {
			if ts.Tags.Get("@fp.Value").IsDefined() || ts.Tags.Get("@fp.With").IsDefined() {
				allFields := applyFields(ts)
				...
				return privateFields.Concat(supers)
			}

			if ts.Tags.Get("@fp.WithPubField").IsDefined() {
				allFields := applyFields(ts)
...
func genPrivateGetters(ctx TaggedStructContext, privateFields fp.Seq[metafp.StructField], genMethod fp.Set[string]) fp.Set[string] {
	...
	privateFields.Foreach(func(f metafp.StructField) {
		...
		fnName := uname
		if ts.GetField(fnName).IsEmpty() && ts.Info.Method.Get(fnName).IsEmpty() && !genMethod.Contains(fnName) {
			ftp := f.TypeName(w, workingPackage)
...
func genMutable(ctx TaggedStructContext, genMethod fp.Set[string]) fp.Set[string] {
	...
	if !isTypeDefined(workingPackage, mutableTypeName) {
		mutableFields := seq.Iterator(ts.Fields).Map(func(v metafp.StructField) string {
			...
			name := fp.Seq[string]{}
			if !v.Embedded {
				fname := publicName(v.Name)
				if fname != v.Name && ts.GetField(fname).IsDefined() {
					fname = fname + "2"
				}
				name = name.Append(fname)
			}
	...
	if ts.Info.Method.Get("AsMutable").IsEmpty() {
		...
		fields := iterator.FromSeq(allFields).Map(func(f metafp.StructField) string {
			...
			return fmt.Sprintf(`%s : r.%s`, fname, f.Name)
		}).MakeString(",\n")
...
func typeParamDecl(w genfp.ImportSet, cwd genfp.WorkingPackage, p fp.Seq[metafp.TypeParam]) string {
	if len(p) > 0 {
		return "[" + seq.Iterator(p).Map(func(v metafp.TypeParam) string {
...
func processValue(ctx TaggedStructContext, genMethod fp.Set[string]) fp.Set[string] {
	...
	if anno, ok := ts.Tags.Get("@fp.Value").Unapply(); ok {
		allFields := applyFields(ts)
		privateFields := ts.Fields.FilterNot(metafp.StructField.Public)
		...
		genMethod = genPrivateGetters(ctx, privateFields, genMethod)
		genMethod = genPublicGetters(ctx, embedField, anno, genMethod)
		genMethod = genPrivateWiths(ctx, privateFields, genMethod)
		genMethod = genPublicWiths(ctx, embedField, anno, genMethod)
