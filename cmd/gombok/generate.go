...
func generateAdaptor(w genfp.Writer, gad generator.GenerateAdaptorDirective) {
	...
	fieldDecl = fieldDecl.Concat(fields.Map(xtr.Head).FilterNot(eq.GivenValue("")))

	fmt.Fprintf(w, "// generated by %s:%d\n", gad.Decl.File, gad.Decl.Line)
	fmt.Fprintf(w, `type %s%s struct {
					%s
				}
				`, adaptorTypeName, decltp, fieldDecl.MakeString("\n"))

	fmt.Fprintf(w, "%s", fields.Map(xtr.Tail).MakeString("\n"))
...
func toStructInfo(is genfp.ImportSet, workingPkg genfp.WorkingPackage, ti metafp.TypeInfo) fp.Try[genfp.StructInfo] {
	...
	if ti.IsStruct() || ti.Underlying().IsStruct() {
		...
		fields := ti.Fields().Map(func(f metafp.StructField) genfp.StructFieldInfo {
...
func toTypeInfo(w genfp.ImportSet, workingPkg genfp.WorkingPackage, v metafp.TypeInfoExpr) genfp.TypeInfo {
	...
	return genfp.TypeInfo{
		...
		AliasTypeArgs: v.Type.TypeArgs.Map(func(v metafp.TypeInfo) genfp.TypeInfo {
			return toTypeInfo(w, workingPkg, metafp.TypeInfoExpr{
				Type: v,
			})
		}),
		TypeArgs: v.Type.Unalias().TypeArgs.Map(func(v metafp.TypeInfo) genfp.TypeInfo {
			return toTypeInfo(w, workingPkg, metafp.TypeInfoExpr{
				Type: v,
			})
		}),
...
func genGenerate() {
	...
	filelist := iterator.ToGoSet(
		mutable.MapOf(gentemplate).Keys().
			...
			Concat(mutable.MapOf(genmonadmethods).Keys()),
	)
	...
	for file := range filelist {
		genfp.Generate(pack, file, func(w genfp.Writer) {
			...
			for _, md := range monadt[file] {
				fmt.Fprintf(w, "// generated by %s:%d\n", md.Decl.File, md.Decl.Line)
				generatedMethod := map[string]bool{}
				if md.TransformerType != nil {
					generatedMethod = methodList[md.TransformerType.GenericType.Obj().Name()]
					if generatedMethod == nil {
						targetType := md.TransformerType.GenericType.Obj().Type()
						generatedMethod = metafp.GetMethodList(pkgs, targetType, fileSet)
					}
				}
				generator.WriteMonadTransformers(w, md, funcList, generatedMethod)
				if md.TransformerType != nil {
					methodList[md.TransformerType.GenericType.Obj().Name()] = generatedMethod
				}
			}

			for _, md := range genmonadmethods[file] {
				generated := methodList[md.TargetType.Obj().Name()]
				if generated == nil {
					targetType := md.TargetType.Obj().Type()
					generated = metafp.GetMethodList(pkgs, targetType, fileSet)
				}
				generator.WriteMonadMethods(w, md, generated)
				methodList[md.TargetType.Obj().Name()] = generated
			}
		})
	}
