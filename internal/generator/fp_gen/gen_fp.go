...
func main() {
	metafp.Generate("fp", "func_gen.go", func(f metafp.Writer) {
		for i := 3; i < max.Func; i++ {
			fmt.Fprintf(f, "type Func%d[%s, R any] func(%s) R\n", i, metafp.FuncTypeArgs(1, i), metafp.FuncDeclArgs(1, i))
		...
		for i := 3; i < max.Func; i++ {
			...
			// 	}
			// }
			// `, i, metafp.FuncTypeArgs(1, i), curriedType(1, i), curriedType(2, i), i-1, metafp.FuncTypeArgs(2, i), metafp.FuncDeclArgs(2, i), metafp.FuncCallArgs(1, i))
			...
			for j := i - 1; j < i; j++ {
			   ...
			   `, i, metafp.FuncTypeArgs(1, i), j, metafp.FuncDeclArgs(1, j), i-j, metafp.FuncTypeArgs(j+1, i), metafp.FuncDeclArgs(j+1, i), metafp.FuncCallArgs(1, i))
			}
			...
			for j := i - 1; j < i; j++ {
			   ...
			   `, i, metafp.FuncTypeArgs(1, i), j, metafp.FuncDeclArgs(i-j+1, i), i-j, metafp.FuncTypeArgs(1, i-j), metafp.FuncDeclArgs(1, i-j), metafp.FuncCallArgs(1, i))
			}
			...
			/* 일부만 아규먼트 적용하는 함수는
			   ...
			   `, i, metafp.FuncTypeArgs(1, i), metafp.FuncDeclArgs(1, 1), i-1, metafp.FuncTypeArgs(2, i), metafp.FuncDeclArgs(2, i), metafp.FuncCallArgs(1, i))
			   ...
			   `, i, metafp.FuncTypeArgs(1, i), metafp.FuncDeclArgs(i, i), i-1, metafp.FuncTypeArgs(1, i-1), metafp.FuncDeclArgs(1, i-1), metafp.FuncCallArgs(1, i))
			   ...
			   `, i, metafp.FuncTypeArgs(1, i), ordinalName[j], metafp.FuncDeclArgs(j, j), i-1, metafp.FuncTypeArgs(1, j-1), metafp.FuncTypeArgs(j+1, i), metafp.FuncDeclArgs(1, j-1), metafp.FuncDeclArgs(j+1, i), metafp.FuncCallArgs(1, i))
			...
			*/
			...
			// 	}
			// }
			// `, i, metafp.FuncTypeArgs(1, i), i, metafp.FuncTypeArgs(2, i), metafp.FuncDeclArgs(2, i), metafp.FuncCallArgs(1, i))
			// 			}
...
func Compose%d[%s,R any] ( %s ) Func1[A1,R] {
	return Compose2(f1, Compose%d(%s))
			...
			`, i, metafp.FuncTypeArgs(1, i), metafp.FuncChain(1, i), i-1, strings.ReplaceAll(metafp.FuncCallArgs(2, i), "a", "f"))
		}
...
func Id%d[%s,R any] ( %s, r R) R {
		return r
			...
			`, i, metafp.FuncTypeArgs(1, i-1), metafp.FuncDeclArgs(1, i-1))
		}
	})

	metafp.Generate("fp", "tuple_gen.go", func(f metafp.Writer) {
