// Code generated by template_gen, DO NOT EDIT.
package fp

// generated by fp.go:491

type Func3[A1, A2, A3, R any] func(A1, A2, A3) R

type Func4[A1, A2, A3, A4, R any] func(A1, A2, A3, A4) R

type Func5[A1, A2, A3, A4, A5, R any] func(A1, A2, A3, A4, A5) R

type Func6[A1, A2, A3, A4, A5, A6, R any] func(A1, A2, A3, A4, A5, A6) R

type Func7[A1, A2, A3, A4, A5, A6, A7, R any] func(A1, A2, A3, A4, A5, A6, A7) R

type Func8[A1, A2, A3, A4, A5, A6, A7, A8, R any] func(A1, A2, A3, A4, A5, A6, A7, A8) R

type Func9[A1, A2, A3, A4, A5, A6, A7, A8, A9, R any] func(A1, A2, A3, A4, A5, A6, A7, A8, A9) R

// generated by fp.go:502

func (r Func3[A1, A2, A3, R]) ApplyFirst2(a1 A1, a2 A2) func(A3) R {
	return func(a3 A3) R {
		return r(a1, a2, a3)
	}
}

func (r Func3[A1, A2, A3, R]) ApplyLast2(a2 A2, a3 A3) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3)
	}
}

func (r Func3[A1, A2, A3, R]) Widen() func(A1, A2, A3) R {
	return r
}

func (r Func4[A1, A2, A3, A4, R]) ApplyFirst3(a1 A1, a2 A2, a3 A3) func(A4) R {
	return func(a4 A4) R {
		return r(a1, a2, a3, a4)
	}
}

func (r Func4[A1, A2, A3, A4, R]) ApplyLast3(a2 A2, a3 A3, a4 A4) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4)
	}
}

func (r Func4[A1, A2, A3, A4, R]) Widen() func(A1, A2, A3, A4) R {
	return r
}

func (r Func5[A1, A2, A3, A4, A5, R]) ApplyFirst4(a1 A1, a2 A2, a3 A3, a4 A4) func(A5) R {
	return func(a5 A5) R {
		return r(a1, a2, a3, a4, a5)
	}
}

func (r Func5[A1, A2, A3, A4, A5, R]) ApplyLast4(a2 A2, a3 A3, a4 A4, a5 A5) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4, a5)
	}
}

func (r Func5[A1, A2, A3, A4, A5, R]) Widen() func(A1, A2, A3, A4, A5) R {
	return r
}

func (r Func6[A1, A2, A3, A4, A5, A6, R]) ApplyFirst5(a1 A1, a2 A2, a3 A3, a4 A4, a5 A5) func(A6) R {
	return func(a6 A6) R {
		return r(a1, a2, a3, a4, a5, a6)
	}
}

func (r Func6[A1, A2, A3, A4, A5, A6, R]) ApplyLast5(a2 A2, a3 A3, a4 A4, a5 A5, a6 A6) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4, a5, a6)
	}
}

func (r Func6[A1, A2, A3, A4, A5, A6, R]) Widen() func(A1, A2, A3, A4, A5, A6) R {
	return r
}

func (r Func7[A1, A2, A3, A4, A5, A6, A7, R]) ApplyFirst6(a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6) func(A7) R {
	return func(a7 A7) R {
		return r(a1, a2, a3, a4, a5, a6, a7)
	}
}

func (r Func7[A1, A2, A3, A4, A5, A6, A7, R]) ApplyLast6(a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4, a5, a6, a7)
	}
}

func (r Func7[A1, A2, A3, A4, A5, A6, A7, R]) Widen() func(A1, A2, A3, A4, A5, A6, A7) R {
	return r
}

func (r Func8[A1, A2, A3, A4, A5, A6, A7, A8, R]) ApplyFirst7(a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7) func(A8) R {
	return func(a8 A8) R {
		return r(a1, a2, a3, a4, a5, a6, a7, a8)
	}
}

func (r Func8[A1, A2, A3, A4, A5, A6, A7, A8, R]) ApplyLast7(a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7, a8 A8) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4, a5, a6, a7, a8)
	}
}

func (r Func8[A1, A2, A3, A4, A5, A6, A7, A8, R]) Widen() func(A1, A2, A3, A4, A5, A6, A7, A8) R {
	return r
}

func (r Func9[A1, A2, A3, A4, A5, A6, A7, A8, A9, R]) ApplyFirst8(a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7, a8 A8) func(A9) R {
	return func(a9 A9) R {
		return r(a1, a2, a3, a4, a5, a6, a7, a8, a9)
	}
}

func (r Func9[A1, A2, A3, A4, A5, A6, A7, A8, A9, R]) ApplyLast8(a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7, a8 A8, a9 A9) func(A1) R {
	return func(a1 A1) R {
		return r(a1, a2, a3, a4, a5, a6, a7, a8, a9)
	}
}

func (r Func9[A1, A2, A3, A4, A5, A6, A7, A8, A9, R]) Widen() func(A1, A2, A3, A4, A5, A6, A7, A8, A9) R {
	return r
}

// generated by fp.go:527

func Compose3[A1, A2, A3, R any](f1 Func1[A1, A2], f2 Func1[A2, A3], f3 Func1[A3, R]) Func1[A1, R] {
	return Compose2(f1, Compose2(f2, f3))
}

func Compose4[A1, A2, A3, A4, R any](f1 Func1[A1, A2], f2 Func1[A2, A3], f3 Func1[A3, A4], f4 Func1[A4, R]) Func1[A1, R] {
	return Compose2(f1, Compose3(f2, f3, f4))
}

func Compose5[A1, A2, A3, A4, A5, R any](f1 Func1[A1, A2], f2 Func1[A2, A3], f3 Func1[A3, A4], f4 Func1[A4, A5], f5 Func1[A5, R]) Func1[A1, R] {
	return Compose2(f1, Compose4(f2, f3, f4, f5))
}

// generated by fp.go:540

func Id2[A1, R any](a1 A1, r R) R {
	return r
}

func Id3[A1, A2, R any](a1 A1, a2 A2, r R) R {
	return r
}

func Id4[A1, A2, A3, R any](a1 A1, a2 A2, a3 A3, r R) R {
	return r
}

func Id5[A1, A2, A3, A4, R any](a1 A1, a2 A2, a3 A3, a4 A4, r R) R {
	return r
}

func Id6[A1, A2, A3, A4, A5, R any](a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, r R) R {
	return r
}

func Id7[A1, A2, A3, A4, A5, A6, R any](a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, r R) R {
	return r
}

func Id8[A1, A2, A3, A4, A5, A6, A7, R any](a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7, r R) R {
	return r
}

func Id9[A1, A2, A3, A4, A5, A6, A7, A8, R any](a1 A1, a2 A2, a3 A3, a4 A4, a5 A5, a6 A6, a7 A7, a8 A8, r R) R {
	return r
}
