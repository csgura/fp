...
func _[A, B any]() genfp.GenerateMonadTransformer[fp.Try[fp.Seq[A]]] {
	return genfp.GenerateMonadTransformer[fp.Try[fp.Seq[A]]]{
		...
		Transform: []any{
			fp.Seq[A].Filter[fp.Phantom[A]],
			fp.Seq[A].Add[fp.Phantom[A]],
			fp.Seq[A].Append[fp.Phantom[A]],
			fp.Seq[A].Concat[fp.Phantom[A]],
			fp.Seq[A].Drop[fp.Phantom[A]],
			fp.Seq[A].Exists[fp.Phantom[A]],
			fp.Seq[A].FilterNot[fp.Phantom[A]],
			fp.Seq[A].Find[fp.Phantom[A]],
			fp.Seq[A].ForAll[fp.Phantom[A]],
			fp.Seq[A].Foreach[fp.Phantom[A]],
			fp.Seq[A].Get[fp.Phantom[A]],
			fp.Seq[A].Head[fp.Phantom[A]],
			fp.Seq[A].Tail[fp.Phantom[A]],
			fp.Seq[A].Init[fp.Phantom[A]],
			fp.Seq[A].IsEmpty[fp.Phantom[A]],
			fp.Seq[A].Last[fp.Phantom[A]],
			fp.Seq[A].MakeString[fp.Phantom[A]],
			fp.Seq[A].NonEmpty[fp.Phantom[A]],
			fp.Seq[A].Reverse[fp.Phantom[A]],
			fp.Seq[A].Size[fp.Phantom[A]],
			fp.Seq[A].Take[fp.Phantom[A]],
			seq.Fold[A, B],
			seq.Scan[A, B],
			seq.Sort[A],
			seq.Min[A],
