...
type GenerateMonadMethodsDirective struct {
	Decl       TaggedLit
	Package    genfp.WorkingPackage
	TargetType GenericType
	// 생성될 file 이름
...
func ParseGenerateMonadMethods(lit TaggedLit) (GenerateMonadMethodsDirective, error) {
	ret := GenerateMonadMethodsDirective{
		Decl:    lit,
		Package: lit.WorkingPackage(),
	}
	...
	names := []string{"File", "TypeParm", "Pure"}
	for idx, e := range lit.Lit.Elts {
		if idx >= len(names) {
			return ret, fmt.Errorf("invalid number of literals")
		}
