...
func TestMonoid(t *testing.T) {
	s := seq.Of(1, 2, 3, 4, 5)
	assert.Equal(seq.Reduce(s, monoid.Sum[int]()), 15)

	assert.Equal(seq.Reduce(s, monoid.Product[int]()), 120)

	s2 := seq.Of("hello", " ", "world")
	assert.Equal(seq.Reduce(s2, monoid.Sum[string]()), "hello world")

	seqseq := seq.Of(seq.Of(1, 2), seq.Of(3, 4), seq.Of(5, 6))
	flatten := seq.Reduce(seqseq, monoid.MergeSeq[int]())
	fmt.Println(flatten)
